...
#from https://github.com/rapid7/metasploit-framework/blob/master/modules/auxiliary/scanner/smb/smb_ms17_010.rb
ms17_010_detect = binascii.unhexlify("0000004aff534d42250000000018012800000000000000000000000000086dbd0108e7c61000000000ffffffff0000000000000000000000004a0000004a0002002300000007005c504950455c00")

# Arguments
parser = argparse.ArgumentParser(description="Detect present of MS17-010 vulnerablity AND DOUBLEPULSAR SMB implant\n\nOriginal Author: Luke Jennings\nWebsite: https://countercept.com\nTwitter: @countercept\nUpdated By: Mike Schladt and Chris Boettcher", formatter_class=argparse.RawTextHelpFormatter)
...
def check_ip(ip):
    ...
    if verbose:
        print_status(ip, "Checking for ms17-010 patch...")

    modified_ms17_010_detect = list(ms17_010_detect)
    modified_ms17_010_detect[28] = tree_id[0]
    modified_ms17_010_detect[29] = tree_id[1]
    modified_ms17_010_detect[32] = user_id[0]
    modified_ms17_010_detect[33] = user_id[1]
    modified_ms17_010_detect = "".join(modified_ms17_010_detect)

    s.send(modified_ms17_010_detect)
    detect_ms17_010_response = s.recv(1024)

    response_code = binascii.hexlify(detect_ms17_010_response)[18:26]

    if response_code == "050200c0" :
        print "[+] [%s] RECEIVED ERROR CODE %s. MACHINE IS VULNERABLE TO ETERNALBLUE (patch ms17-010)!" % (ip, response_code)
    else :
        print "[-] [%s] Received code %s. Machine is likely patched for ms17-010" % (ip, response_code)
